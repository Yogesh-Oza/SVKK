...
} from "./policy-csv-export-columns";
...
describe("policy CSV export column groups", () => {
  it("excludes commission columns without permission", () => {
    ...
    const keys = expandExportColumnSelection(groups, allExportUiKeys(groups));
  ...
  it("shows each payment and member field once, expanding to every slot", () => {
    ...
    expect(allExportUiKeys(groups).length).toBeLessThan(120);
    expect(allExportUiKeys(groups).length).toBeGreaterThan(40);
  });
