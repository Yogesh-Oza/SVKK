...
const SORTS: Record<string, Prisma.PolicyOrderByWithRelationInput | Prisma.PolicyOrderByWithRelationInput[]> = {
  ...
  // Prefer policy holder snapshots (displayed value); party column is fallback for null snapshots.
  name: [{ holderName: "asc" }, { insuredParty: { name: "asc" } }],
  "-name": [{ holderName: "desc" }, { insuredParty: { name: "desc" } }],
  name_desc: [{ holderName: "desc" }, { insuredParty: { name: "desc" } }],
  village: { village: "asc" },
  ...
  customerId: [{ holderCustomerId: "asc" }, { insuredParty: { customerId: "asc" } }],
  customerId_desc: [{ holderCustomerId: "desc" }, { insuredParty: { customerId: "desc" } }],
  ...
  mobile: [{ holderMobile: "asc" }, { insuredParty: { mobile: "asc" } }],
  mobile_desc: [{ holderMobile: "desc" }, { insuredParty: { mobile: "desc" } }],
  /** SVKK ID column: insured party public id (shared identity — not snapshotted) */
  ...
  const searchWhere: Prisma.PolicyWhereInput | undefined = s
    ? {
        OR: [
          ...
          { holderCustomerId: containsInsensitive(s) },
          { holderEmail: containsInsensitive(s) },
          { holderMobile: containsInsensitive(s) },
          ...
          { insuredParty: { email: containsInsensitive(s) } },
