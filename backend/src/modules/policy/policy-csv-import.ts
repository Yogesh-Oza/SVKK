...
async function updatePolicyCsvRow(
  ...
  const loanRepayment = getCsvField(map, "loan_repayment");
  if (loanRepayment) policyUpdate.loanRepaymentAmount = parseOptionalDecimal(loanRepayment);
  const loanPending = getCsvField(map, "loan_pending_amt");
  if (loanPending) policyUpdate.loanPendingAmount = parseOptionalDecimal(loanPending);
  ...
  const nomineeDob = getCsvField(map, "nominee_dob");
  if (nomineeDob) policyUpdate.nomineeDateOfBirth = parseOptionalDate(nomineeDob);
  const bankAcHolder = getCsvField(map, "bank_ac_holder_name");
  if (bankAcHolder) policyUpdate.policyBankHolderName = bankAcHolder;
  const bankAcNo = getCsvField(map, "bank_ac_no");
  if (bankAcNo) policyUpdate.policyBankAccountNo = bankAcNo;
  const bankIfsc = getCsvField(map, "bank_ifsc");
  if (bankIfsc) policyUpdate.policyBankIfsc = bankIfsc;
  const bankBranch = getCsvField(map, "bank_branch");
  if (bankBranch) policyUpdate.policyBankBranch = bankBranch;
  const bankName = getCsvField(map, "bank_name");
  if (bankName) policyUpdate.policyBankName = bankName;
