...
export function transformPolicyRow(row: LegacyPolicyRow): TransformedPolicy {
  ...
  const policyData = {
    ...
    nomineeDateOfBirth: parseDateSafe(row.nominee_dob),
    loanRef: null,
    ...
    loanRepaymentAmount: parseDecimalSafe(row.loan_repayment),
    loanPendingAmount: parseDecimalSafe(row.loan_pending_amt),
    policyBankHolderName: row.bank_ac_holder_name?.trim() || null,
    policyBankAccountNo: row.bank_ac_no?.trim() || null,
    policyBankIfsc: row.bank_ifsc?.trim() || null,
    policyBankBranch: row.bank_branch?.trim() || null,
    policyBankName: row.bank_name?.trim() || null,
