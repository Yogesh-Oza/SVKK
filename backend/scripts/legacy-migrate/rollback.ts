...
export async function rollbackMigrationRun(
  ...
  if (dryRun) {
    ...
    counts.parties = await prisma.insuredParty.count({
      where: { createdInMigrationRunId: migrationRunId },
    });
    return counts;
  }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const parties = await tx.insuredParty.findMany({
      where: { createdInMigrationRunId: migrationRunId },
      select: { id: true },
    });
    for (const p of parties) {
      const remaining = await tx.policy.count({ where: { insuredPartyId: p.id } });
      if (remaining === 0) {
        await tx.insuredParty.delete({ where: { id: p.id } });
        counts.parties += 1;
      }
    }
